...
#    predictors = np.column_stack((NB[:,0],lc[:,1],lc[:,2],lcH[:,0],lcH[:,1],lcH[:,2],tau[:,0],tau[:,1],sa[:,0],sa[:,1],bv[:,0],bv[:,1],CoHOMO[:,0],CoHOMO[:,0],CoHOMO[:,2],CoLUMO[:,0],CoLUMO[:,1],CoLUMO[:,2],CoH2Len[:,0],CoH2Len[:,1],CoH2Len[:,2],CoH2Len[:,3],ba[:,0],ba[:,1],ba[:,2],ba[:,3],ba[:,4],ba[:,5],lt[:,0],lt[:,1],lt[:,2],lt[:,3],lt[:,4],lt[:,5],lb[:,0],lb[:,1],lb[:,2],lb[:,3],lb[:,4],lb[:,5]))
#    predictors = np.column_stack((asa,abv,aba,abb,abt,NBO[:,0],NBO[:,1],NBO[:,2],lcH[:,0],lcH[:,1],lcH[:,2],tau[:,0],tau[:,1],CoH2Len[:,0],CoH2Len[:,1],CoH2Len[:,2],nlmo))
...
#    predictors = np.column_stack((abv,svr,st,B5,CoH2Len[:,2]))
#    predictors = np.column_stack((abv,svr,st,B5,tau[:,0],tau[:,1]))
#    predictors = np.column_stack((L,B1,NBO[:,1],tau[:,1],nlmo))


    predictors = np.column_stack((nlmo)).reshape((-1,1))
#    predictors = np.column_stack((nlmo,NBOH))
...
#    regressor = make_pipeline(polyFeatures, StandardScaler(), Lasso(alpha=0.1, max_iter=70000))#, fit_intercept=True))
...
#    scaler=regressor.steps[1][1].scale_
    for item in regressor.steps[1][1].coef_:
...
#    print('Alpha',regressor.steps[2][1].alpha_)
    ...
    yint=(regressor.steps[1][1].intercept_)
 #   print('Intercept:',regressor.steps[2][1].intercept_)
 #   print('Intercept:',regressor.steps[2][1].intercept_)
...
#    print("linear Regression", r_value**2)
    ...
    OtherPlotVar="CoH-NLMO X 100"
#### Graphing the model versus prediction section ####
#    if hydricities[0]==therm[0,0]:
#        scatterPlot1.pka(hydricities, predictions, yadjust, Path('D:/' '/GoogleDrive' '/Desktop' '/ML_Figures' '/PkaPrediction'))
#        print('pKa')
    if hydricities[0]==therm[0,0]: 
        scatterPlot1.Hyd(hydricities, predictions, nlmo, slope, yint, Path('D:/' '/GoogleDrive' '/Desktop' '/ML_Figures' '/HydricityPrediction'))
        print('Hydricity')
    if hydricities[0]==therm[0,2]:
        scatterPlot1.h2binding(hydricities, predictions, Path('D:/' '/GoogleDrive' '/Desktop' '/ML_Figures' '/H2Binding'))
        print('H2Binding')
#    else:
