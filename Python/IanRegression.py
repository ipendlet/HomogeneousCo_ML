...
#    dataFile = Path('D:/' '/GoogleDrive' '/Desktop' '/ML_Figures' '/AllData_CH3CN_Alkyl.csv')
    dataFile = Path('/' 'mnt/' 'd/' '/GoogleDrive' '/Desktop' '/ML_Figures' '/AllData_CH3CN_Alkyl.csv')
    print(dataFile)
...
#Steric Properties
   ...
   #surface area :q
    ...
    asa = np.loadtxt(dataFile,delimiter=',',skiprows=1,usecols=(103))
   ...
   #biteangle
    ...
    abv = np.loadtxt(dataFile,delimiter=',',skiprows=1,usecols=(102))
   #Ligand Torsion
...
#    predictors = np.column_stack((lc[:,0],lc[:,1],lc[:,2],lcH[:,0],lcH[:,1],lcH[:,2],tau[:,0],tau[:,1],sa[:,0],sa[:,1],bv[:,0],bv[:,1],CoHOMO[:,0],CoHOMO[:,0],CoHOMO[:,2],CoLUMO[:,0],CoLUMO[:,1],CoLUMO[:,2],CoH2Len[:,0],CoH2Len[:,1],CoH2Len[:,2],CoH2Len[:,3],ba[:,0],ba[:,1],ba[:,2],ba[:,3],ba[:,4],ba[:,5],lt[:,0],lt[:,1],lt[:,2],lt[:,3],lt[:,4],lt[:,5],lb[:,0],lb[:,1],lb[:,2],lb[:,3],lb[:,4],lb[:,5]))

#Alkyl  - NewStoryLine
#    predictors = np.column_stack((NBO[:,0],NBO[:,2],lcH[:,0],lcH[:,1],tau[:,1],abv,CoHOMO[:,1],CoHOMO[:,0]))
    predictors = np.column_stack((lcH[:,2],tau[:,1],abv,CoHOMO[:,1]))
...
# Predicting HOMO
#    predictors = np.column_stack((NBO[:,1],NBO[:,2],lcH[:,0],lcH[:,1],lcH[:,2],tau[:,0],tau[:,1],asa,CoH2Len[:,0],CoH2Len[:,1],CoH2Len[:,2]))
#    predictors = np.column_stack((NBO[:,1],lcH[:,2],tau[:,1],asa,CoH2Len[:,2]))
...
#    print("linear Regression", r_value**2)
    ...
    OtherPlotVar="CoH-HOMO"
#### Graphing the model versus prediction section ####
    if hydricities[0]==therm[0,0]:
        scatterPlot.pka(hydricities, predictions, Path('/' 'mnt/' 'd' '/GoogleDrive' '/Desktop' '/ML_Figures' '/PkaPrediction'))
        print('pKa')
    if hydricities[0]==therm[0,1]: 
        scatterPlot.Hyd(hydricities, predictions, Path('/' 'mnt/' 'd' '/GoogleDrive' '/Desktop' '/ML_Figures' '/HydricityPrediction'))
        print('Hydricity')
    if hydricities[0]==therm[0,2]:
        scatterPlot.h2binding(hydricities, predictions, Path('/' 'mnt/' 'd' '/GoogleDrive' '/Desktop' '/ML_Figures' '/H2Binding'))
        print('H2Binding')
    else:
        scatterPlot.other(hydricities, predictions, Path('/' 'mnt/' 'd' '/GoogleDrive' '/Desktop' '/ML_Figures' '/OtherPrediction'), OtherPlotVar)
