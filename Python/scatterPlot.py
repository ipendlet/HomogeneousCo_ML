...
def other(actual, predicted, outFileName, thing):
    ...
    # make plot square with equal x and y axes
    bounds = [min(list(actual) + list(predicted))-0.02, max(list(actual) + list(predicted))+0.02]
    plt.axis(bounds * 2)
    axes.set_aspect('equal', adjustable='box')
...
#    print(rSquared)
    plt.figtext(0.6,0.15,'$R^2 = $'+format(rSquared,'.3f'), fontsize=12)
    plt.xlabel('QM Calculated NLMO Energy (eV)', fontsize=14)
    plt.ylabel('Predicted NLMO Energy (eV)', fontsize=14)
    plt.title('Calculated vs. Predicted Co-H NLMO Energy', fontsize=14)
#    plt.tight_layout()
    plt.savefig(str(outFileName) + '.png')
    plt.clf()
